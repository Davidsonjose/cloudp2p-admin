...
// import { saveAs } from "file-saver";
function AccountSettingsManager() {
  ...
  const handleCopy = () => {
    ...
    navigator.clipboard
      .writeText(userData?.payload[0]?.token?.originalcode)
      .then(
        function () {
          setCopy(true);
        },
  ...
  const downloadQR = () => {
    saveAs(`${userData?.payload[0]?.token?.qrcode}`, "lfzqrcode.jpg"); // Put your image url here.
  };
        ...
        <>
          ...
          <div className="">
            <div className="flex justify-center">
              <img
                src={userData && userData[0]?.token?.qrcode}
                alt=""
                id="lfzentry"
              />
            </div>
            <div className="">
              ...
              <h4 className="flex items-center text-center justify-center font-black text-[50px]">
                {userData && userData[0]?.token?.originalcode}
              </h4>
              ...
              <div className="pt-2 flex items-center space-x-2 justify-center">
                ...
                {copy ? (
                  ...
                  <small className="font-semibold">Copy code</small>
                )}
              </div>
            </div>
          </div>
          <p className="text-red-600 mb-4 font-bold text-center">
            valid until{" "}
            {moment(userData && userData[0]?.token?.expiresAt).format(
