...
function Dashboard() {
  ...
  useEffect(() => {
    axiosCreate
      .get(`${API_URL}/admin/messages`)
      .then(({ data }: any) => {
        const comm = data?.payload[0];
        setCommunication(comm?.length);
      })
      .catch((err) => {
        console.log(err);
      });
    axiosCreate
      .get(`${API_URL}/admin/information`)
      .then(({ data }: any) => {
        const comm = data?.payload[0];
        setInformation(comm?.length);
      })
      .catch((err) => {
        console.log(err);
      });
    axiosCreate
      .get(`${API_URL}/code/monthly`)
      .then(({ data }: any) => {
        const comm = data?.payload;
        if (comm?.length === 0) {
          setMonthly(0);
        } else {
          setMonthly(comm[0]?.total);
        }
      })
      .catch((err) => {
        console.log(err);
    ...
    axiosCreate
      .get(`${API_URL}/code/monthly-expired`)
      .then(({ data }: any) => {
        const comm = data?.payload;
    ...
    axiosCreate
      .get(`${API_URL}/code/yearly`)
      .then(({ data }: any) => {
        const comm = data?.payload;
        if (comm?.length === 0) {
          setYearly(0);
        } else {
          setYearly(comm[0]?.total);
        }
      })
      .catch((err) => {
        console.log(err);
    ...
    axiosCreate
      .get(`${API_URL}/code/active`)
      .then(({ data }: any) => {
        const comm = data?.payload[0];
        setActive(comm?.length);
      })
      .catch((err) => {
        console.log(err);
      });
